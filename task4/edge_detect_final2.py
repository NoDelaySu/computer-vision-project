...
ret, binary = cv2.threshold(gaussianBlur, 100, 127, cv2.THRESH_BINARY)
...
# Canny算子
canny_img = cv2.resize(img, None, fx=0.8, fy=0.8, interpolation=cv2.INTER_CUBIC)
canny_img = cv2.GaussianBlur(canny_img, (3, 3), 0)
Canny = cv2.Canny(canny_img, 50, 150, apertureSize=3)
...
# #           Scharr, Canny, LOG]
titles = ['Source Image', 'Binary Image', 'Roberts Image',
          'Prewitt Image', 'Sobel Image', 'Kirsch Image',
          'Scharr Image', 'Canny Image', 'LOG Image']
images = [lenna_img, binary, Roberts,
          Prewitt, Sobel, Laplacian,
          Scharr, Canny, LOG]

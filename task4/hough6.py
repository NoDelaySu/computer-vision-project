...
if __name__ == "__main__":
    imgPath = "./data/test1.jpeg"
    I = cv2.imread(imgPath, cv2.IMREAD_GRAYSCALE)
    if len(sys.argv) > 1:
        # 输入图像
    ...
    for r in range(rows):
        for c in range(cols):
            if accumulator[r][c] > voteThresh:
                points = accuDict[(r, c)]
                cv2.line(I, points[0], points[len(points) - 1], (255, 0, 0), 1)
    cv2.imshow('accumulator', grayAccu)

    # 显示原图
    cv2.imshow("I", I)
    plt.show()
    cv2.imwrite('./data/accumulator.jpg', grayAccu)
    cv2.imwrite('./data/I.jpg', I)
    cv2.waitKey(0)

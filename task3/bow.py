...
class BOW(object):
    ...
    def fit(self, train_path, k):
        ...
        # FLANN匹配  参数algorithm用来指定匹配所使用的算法，可以选择的有
        # LinearIndex、KTreeIndex、KMeansIndex、CompositeIndex和AutotuneIndex，
        # 这里选择的是KTreeIndex(使用kd树实现最近邻搜索)
        flann_params = dict(algorithm=1, tree=5)
        flann = cv2.FlannBasedMatcher(flann_params, {})
        # 创建BOW训练器，指定k-means参数k   把处理好的特征数据全部合并，
        # 利用聚类把特征词分为若干类，此若干类的数目由自己设定，每一类相当于一个视觉词汇
...
if __name__ == '__main__':
    ...
    for index in range(begin_index, begin_index + test_samples):
        cat = './tests/data/cats/cat.{0}.jpg'.format(index)
        # print(cat)

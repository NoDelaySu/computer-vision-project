...
class FisherFace(object):
            ...
            for dirname in dirnames:
                for subParent,subDirName,subFilenames in os.walk(parent+'/'+dirname):
                    ...
                    for index,filename in enumerate(subFilenames):
                        ...
                        label.append(subParent[9:]) # 截取字符作为标签
                        dataList.append(dataMat)
                        # print(subParent,dataMat)
    ...
    def LDA(self,dataList,k):
        ...
        eigVals, eigVects = np.linalg.eig(np.mat(np.linalg.pinv(Sw)*Sb))
    ...
    def compare(self,dataList,testImg,label):
        ...
        # print(sortIndex[0])
        # print(len(label))
        return label[int(sortIndex[0]/26)]
...
if __name__=="__main__":
    ...
    ans = fisherface.predict('./trains', './tests/face/face.png')
    img = cv2.imread('./tests/face/face.png')
    cv2.putText(img,ans,(10,20),font,0.6,(0,255,0),2,cv2.LINE_AA)
